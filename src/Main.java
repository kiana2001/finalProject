...
public class Main {
    public static Enum Maches(String string) {
        ...
        if (string.matches("\\s*[\\w|\\d]+\\s*[=]\\s*[\\w|\\d]+\\s*[\\-*\\/+]\\s*[\\w|\\d]"))
            return Parts.MATH_OPERATION;
        if (string.matches("\\s*print\\s+[\\w+|\\d]+\\s*"))
            return Parts.PRINT;
        if (string.matches("\\s*\\w+\\s*[=]\\s*[\\w|\\d]+\\s*([\\-*\\/+]\\s*[\\w|\\d])*")) {
   ...
   static String x;
    public static String randString() {
        ...
        x = random.ints(leftLimit, rightLimit + 1)
                .limit(targetStringLength)
                .collect(StringBuilder::new, StringBuilder::appendCodePoint, StringBuilder::append)
                .toString();
        return x;
    }
    public static String Samerandom() {
        return  x;

    }
    public static void Changer(String string) {
...
//print x >
        ...
        } else if (Parts.MATH_OPERATION.equals(maches)) {//w=w+y
            String replacestring2 = string.replaceAll("\\s*([\\w|\\d])+\\s*([=])\\s*([\\w|\\d])+\\s*([\\-*\\/+])\\s*([\\w|\\d])", "$1$2$3$4$5;");
            System.out.println(replacestring2);
        } else if (Parts.PRINT.equals(maches)) {
            String replacestring = string.replaceAll("\\s*(print)\\s+([\\w+|\\d])+\\s*", "System.out."+"$1"+"ln."+"($2);");
            System.out.println(replacestring);
        } else if (Parts.ASSIGNEMNT.equals(maches)) {
            String replacestring5 = string.replaceAll("\\s*(\\w+)\\s*([=])\\s*([\\w|\\d]+)\\s*([\\-*\\/+]\\s*[\\w|\\d]*)", "$1$2$3$4;");
            System.out.println(replacestring5);
        } else if (Parts.FOR_HEAD.equals(maches)) {
            String replacestring6 = string.replaceAll("\\s*(for)(\\s+)(\\d+)\\s*", "$1" + "(int$2" + randString() + ";" +Samerandom() +"<="+"$3;" + Samerandom() + "++){");
